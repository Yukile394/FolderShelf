...
import com.yukile.foldershelf.data.repository.AddItemResult
...
class FloatingOverlayService : Service() {
    ...
    /**
     * Balonun durum rengini degistirir: yesil = basarili, kirmizi = hata,
     * amber/turuncu = "zaten eklenmisti".
     */
    private fun setBubbleStatus(colorRes: Int) {
    ...
    private fun resetBubbleStatus() {
        setBubbleStatus(R.color.primary_light)
    }

    private fun flashBubbleError() {
        setBubbleStatus(R.color.error_light)
        bubbleBinding?.root?.postDelayed({ resetBubbleStatus() }, 1500L)
    }

    /**
     * Klasor/dosya basariyla eklendiginde kisa bir "zip" (buyu-kucul)
     * animasyonu ve yesil parlama ile onay verir. GitHub'daki basarili
     * yukleme geri bildirimine benzer, akici ve gecikmesiz bir his icin
     * kisa sureli (150ms) OvershootInterpolator kullanilir.
     */
    private fun flashBubbleSuccess() {
        setBubbleStatus(R.color.success_light)
        val root = bubbleBinding?.root
        root?.animate()
            ...
            ?.start()
        root?.postDelayed({ resetBubbleStatus() }, 1200L)
    }

    /**
     * "Zaten eklenmisti" durumunda amber renkle kisa bir uyari verir
     * (hata degil, bilgilendirme).
     */
    private fun flashBubbleAlreadyExists() {
        setBubbleStatus(R.color.warning_light)
        bubbleBinding?.root?.postDelayed({ resetBubbleStatus() }, 1500L)
    }
        ...
        return try {
            when (event.action) {
                ...
                DragEvent.ACTION_DRAG_ENTERED -> {
                    highlightBubbleForDrag(entered = true)
                    true
                }
                DragEvent.ACTION_DRAG_EXITED -> {
                    highlightBubbleForDrag(entered = false)
                    true
                }
    ...
    /**
     * Surukleme + simgenin uzerine geldiginde: buyur, tam opak olur ve
     * renk parlamasi (glow) verir. Ayrildiginda yumusakca eski haline doner.
     * Akici (gecikmesiz) his icin kisa sureler (120-150ms) kullanilir.
     */
    private fun highlightBubbleForDrag(entered: Boolean) {
        val root = bubbleBinding?.root ?: return
        if (entered) {
            setBubbleStatus(R.color.drop_highlight_light)
        } else {
            resetBubbleStatus()
        }
        root.animate()
            .alpha(if (entered) 0.9f else 1f)
            .scaleX(if (entered) 1.25f else 1f)
            .scaleY(if (entered) 1.25f else 1f)
            .setDuration(130L)
            .setInterpolator(if (entered) OvershootInterpolator(1.5f) else android.view.animation.DecelerateInterpolator())
            .start()
    }
    ...
    private fun handleDrop(event: DragEvent) {
        ...
        for (i in 0 until clipData.itemCount) {
            ...
            try {
                ...
                serviceScope.launch {
                    try {
                        val result = repository.addItemFromUri(applicationContext, uri, ItemType.FOLDER)
                        when (result) {
                            AddItemResult.ADDED -> {
                                flashBubbleSuccess()
                                showAddedNotice(alreadyExisted = false)
                            }
                            AddItemResult.ALREADY_EXISTS -> {
                                flashBubbleAlreadyExists()
                                showAddedNotice(alreadyExisted = true)
                            }
                        }
                    } catch (e: Exception) {
    ...
    /**
     * Klasor eklendiginde/guncellendiginde kisa omurlu bir bildirim
     * gosterir; kullaniciya balonun rengine ek olarak acik bir onay verir.
     */
    private fun showAddedNotice(alreadyExisted: Boolean) {
        try {
            val text = if (alreadyExisted) {
                getString(R.string.drop_already_exists_toast)
            } else {
                getString(R.string.drop_zone_added_toast)
            }
            val notification = NotificationCompat.Builder(this, Constants.NOTIFICATION_CHANNEL_ID)
                ...
                .build()
            getSystemService(NotificationManager::class.java)?.notify(Constants.ADD_NOTICE_ID, notification)
        } catch (e: Exception) {
            Log.e(TAG, "showAddedNotice hata", e)
        }
    }
